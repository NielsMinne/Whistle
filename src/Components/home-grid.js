...
class HomeGridComponent extends Component {
  constructor() {
    super({
      name: 'home-grid',
      routerPath: '/home-grid',
    });
    this.user = new User();
    ...
    this.componentContainer.appendChild(
      Elements.createContainer({
        ...
        children: [
          ...
          Elements.createIcon({
            className: 'fas fa-list fa-2x home-list__grid-icon',
            onClick: () => {
              localStorage.removeItem('gridPref');
              Router.navigate('/home');
            },
          }),
          Elements.createContainer({
            className: 'home-grid__container',
            children: [
